/**
 * canonHeroesData.ts — Master Canon Heroes Database (All 29 Heroes, Ch. 1 - 1509)
 * Trấn Quốc Phò Mã Gia (镇国驸马爷)
 * 
 * Single Source of Truth for:
 * - 4 Primary Classes: Võ Tướng, Mưu Thần, Văn Thần / Năng Thần, Tuyệt Thế Nữ Thần / Kỳ Nhân Dị Sĩ
 * - Canonical Battlefield Roles: Tiên Phong, Thống Soái, Thiết Vệ, Hãm Trận, Thần Xạ, Độc Sĩ, Quân Sư, Vương Tá, Nội Chính, Thần Y, Mật Thám, Thiên Mệnh...
 * - Canon Upgrades (Vàng / Kim): Thần Binh, Vũ Kỹ, Thiên Phú, Ngựa Thần, Vũ Khố
 * - Realm derivation by Martial Force (<90: Vương Giả, 90-99: Hoàng Giả, 100-149: Đế Cấp, 150+: Bán Thánh)
 */

export interface CanonWeaponConfig {
  name: string;
  costGold: number;
  forceBonus: number;
...
export interface CanonSkillConfig {
  name: string;
  costGold: number;
  forceBonus: number;
}

export interface CanonTalentConfig {
  name: string;
  costGold: number;
  forceBonus: number;
  description: string;
...
export interface CanonMountConfig {
  name: string;
  costGold: number;
  percentBonus?: number;
  forceBonus: number;
...
export interface CanonStatsConfig {
  force: number;
  command: number;
  intelligence: number;
  politics: number;
...
export interface CanonHeroConfig {
  id: string;
  name: string;
  courtesyName: string;
  primaryClass: string;
  role: string;
  classRoleBadge: string;
  summonTier: string;
  chapterSummon: number;
  loyalty: number;
  baseStats: CanonStatsConfig;
  weapon: CanonWeaponConfig;
  skills: CanonSkillConfig[];
  specialTalent: CanonTalentConfig;
  mount: CanonMountConfig;
  troopType: string;
  quote: string;
  lore: string;
  hostReturnForce: number;
  avatarUrl?: string;
  splashUrl?: string;
  rarity?: string;
  sealGlyph?: string;
...
import { MartialRealm } from './types';

export function getMartialRealm(force: number): MartialRealm {
  if (force < 80) return 'Phàm Nhân';
  if (force < 90) return 'Vương Giả Cảnh';
  if (force < 100) return 'Hoàng Giả Cảnh';
  if (force < 120) return 'Đế Cảnh';
  if (force < 130) return 'Bán Thánh Cảnh';
  if (force < 140) return 'Thánh Cảnh';
  if (force < 160) return 'Bán Tiên Cảnh';
  return 'Nhân Tiên Cảnh';
}

export const CANON_HEROES: CanonHeroConfig[] = [
  {
    "id": "trieu_van",
    "name": "Triệu Vân",
...
export const HERO_ASSET_LOOKUP: Record<string, { avatarUrl: string; splashUrl: string }> = {
  trieu_van: {
...
export const CANON_HEROES_BY_ID: Record<string, CanonHeroConfig> = {};
for (const h of CANON_HEROES) {
  if (!h.rarity) {
    if (h.baseStats.force >= 95 || h.id === 'lu_bo' || h.id === 'gia_cat_luong' || h.id === 'dieu_thuyen') {
      h.rarity = 'UR';
    } else if (h.baseStats.force >= 85 || h.id === 'trieu_van' || h.id === 'quan_vu' || h.id === 'co_hu' || h.id === 'dien_vi') {
      h.rarity = 'SSR';
    } else if (h.baseStats.force >= 70 || h.id === 'cao_thuan') {
      h.rarity = 'SR';
    } else {
      h.rarity = 'R';
    }
  ...
  h.baseForce = h.baseStats.force;
  h.baseCommand = h.baseStats.command;
  h.baseIntelligence = h.baseStats.intelligence;
  h.title = h.role || h.primaryClass;
  h.realmName = getMartialRealm(h.baseStats.force);
  h.sealGlyph = h.name.slice(0, 1);
  CANON_HEROES_BY_ID[h.id] = h;
}

export const HERO_ID_ALIASES: Record<string, string> = {
  hero_zhaoyun: 'trieu_van',
  hero_zhangliao: 'truong_lieu',
  zhangliao: 'truong_lieu',
  ...
  diaochan: 'dieu_thuyen',
  hero_machao: 'ma_sieu',
  machao: 'ma_sieu',
  hero_guanyu: 'quan_vu',
  guanyu: 'quan_vu',
  hero_huangzhong: 'hoang_trung',
  huangzhong: 'hoang_trung',
  hero_xunyu: 'tuan_uc',
  xunyu: 'tuan_uc',
  hero_zhugeliang: 'gia_cat_luong',
  zhugeliang: 'gia_cat_luong',
  hero_lvbu: 'lu_bo',
  lvbu: 'lu_bo',
  hero_simayi: 'tu_ma_y',
  simayi: 'tu_ma_y',
...
export function resolveCanonHero(idOrAlias: string): CanonHeroConfig | undefined {
  const resolvedId = HERO_ID_ALIASES[idOrAlias] || idOrAlias;
  return CANON_HEROES_BY_ID[resolvedId];
}
